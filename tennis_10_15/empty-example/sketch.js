...
function draw() {
  ...
  strokeWeight(4);
  noFill();
  rect(50, 50, width - 100, height - 100); // コートの枠線
  line(width / 2, 50, width / 2, height - 50); // コートの中央のネット
  ...
  if (!gameIsOver) {
    ...
    textSize(32);
    fill(255);
    text(leftScore + " - " + rightScore, width / 2-40, 45); // 得点を表示
    ...
    if (leftScore >= 10 || rightScore >= 10) {
      ...
      textSize(64);
      fill(255);
      text("Game finish!!", width / 2 - 185, height / 2);
    }
  } else {
...
class Paddle {
  constructor(isLeft) {
    ...
    this.y = height / 2 - this.height / 2; // パドルの初期位置（垂直方向）
  ...
  show() {
    fill(255);
    rect(this.isLeft ? 50 : width - this.width - 50, this.y, this.width, this.height); // パドルを描画
  }
  
  update() {
    this.y += this.ySpeed; // パドルの位置を更新
    this.y = constrain(this.y, 50, height - 50 - this.height); // パドルがコート内に収まるように制限
  }
...
class Ball {
  constructor() {
    this.size = 20; // ボールの直径
    this.x = width / 2; // ボールの初期位置（水平方向）
    this.y = height / 2; // ボールの初期位置（垂直方向）
  ...
  // ボールがパドルに当たったかどうかを判定
  hits(paddle) {
    if (this.x > (paddle.isLeft ? 50 : width - paddle.width - 50) && this.x < (paddle.isLeft ? 50 + paddle.width : width - 50)) {
